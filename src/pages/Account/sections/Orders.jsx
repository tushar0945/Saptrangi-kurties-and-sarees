...
// //       try {
// //         const res = await api.get("/orders/"); // 🔥 backend ready
// //         console.log(res);
...
//         <div className="absolute -bottom-20 -right-16 w-64 h-64 bg-secondary/20 rounded-full blur-3xl"></div> */}
...
//         <h2 className="text-3xl font-bold text-dark mb-2">No Orders Yet</h2>

//         <p className="text-dark/70 text-center max-w-md mb-8">
...
const Orders = () => {
  ...
  if (!orders || orders.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center bg-neutral px-6 py-12">
        <img
          src="https://cdn-icons-png.flaticon.com/512/5626/5626436.png"
          alt="empty-orders"
          className="w-28 mb-6 drop-shadow"
        ...
        <button
          onClick={() => navigate("/")}
          className="px-10 py-3 bg-primary hover:bg-secondary text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition"
        >
          Start Shopping
        </button>
      </div>
  ...
  return (
    <div className="space-y-6">
      {orders.map((order) => {
        const { label, icon } = getStatusUI(order.status);
